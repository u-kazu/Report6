...
public class SlackSlashCommand {
    ...
    @Value("${slashCommandToken}") // application propertiesのTokenのキーの指定
     ...
     * @return
     */
    @RequestMapping(value = "/slash-command", // Slash Commandsによって呼ばれるURLのパス
    ...
    public RichMessage onReceiveSlashCommand(@RequestParam("token") String token,
        ...
        // コマンドと設定したTokenが同じかどうかを判定
        if (!token.equals(slackToken)) {
            return new RichMessage("Sorry! You're not lucky enough to use our slack command."); // 一致しなかった時の表示
        }
